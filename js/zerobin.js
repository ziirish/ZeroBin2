...
function compress(message) {
    return Base64.toBase64( Base64.utob(message) );
}
...
function decompress(data) {
    return Base64.btou( Base64.fromBase64(data) );
}
...
function zeroCipher(key, message) {
    ...
    return compress(encrypted.toString());
}
/**
...
function zeroDecipher(key, data) {
    var decrypted = CryptoJS.AES.decrypt(decompress(data), key).toString(CryptoJS.enc.Utf8);
    return decrypted;
}
...
function send_data() {
    ...
    var randomkey = CryptoJS.lib.WordArray.random(128/8);
    randomkey = randomkey.toString(CryptoJS.enc.Base64);
    randomkey = randomkey.substring(0, randomkey.length - 1);
